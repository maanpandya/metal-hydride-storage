...
function setup_grid(h = 0.05)
    ...
    nodes_1 = 50 # Number of nodes along the first edge
    nodes_2 = 25 # Number of nodes along the second edge
    ...
    gmsh.model.geo.mesh.setTransfiniteCurve(1, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(12, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(2, nodes_2)
    gmsh.model.geo.mesh.setTransfiniteCurve(10, nodes_2)
    
    gmsh.model.geo.mesh.setTransfiniteCurve(12, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(13, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(3, nodes_2)
    gmsh.model.geo.mesh.setTransfiniteCurve(11, nodes_2)
    
    gmsh.model.geo.mesh.setTransfiniteCurve(13, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(5, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(4, nodes_2)
    gmsh.model.geo.mesh.setTransfiniteCurve(6, nodes_2)
    
    gmsh.model.geo.mesh.setTransfiniteCurve(9, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(7, nodes_1)
    gmsh.model.geo.mesh.setTransfiniteCurve(11, nodes_2)
    gmsh.model.geo.mesh.setTransfiniteCurve(8, nodes_2)
...
function setup_constraints(dh, fvp)
    ...
    # inlet_bc = Dirichlet(:u, Γin, (x,t) -> [1.0, 0.0], [1,2])
    # add!(ch, inlet_bc)

    # Walls: Dirichlet BC for velocity
    Γwall = getfacetset(dh.grid, "wall")
    wall_bc = Dirichlet(:u, Γwall, (x,t) -> [0.0, 0.0], [1,2])
    add!(ch, wall_bc)
...
function apply_pressure_inlet!(f, dh, fvu, pressure_value::Float64)
    ...
    for (cellid, facetid) in Γinlet
        cell = getcells(dh.grid)[cellid]
        coords = zeros(Vec{2}, length(cell.nodes))
        getcoordinates!(coords, dh.grid, cellid)
        celldofs!(element_dofs, dh, cellid)
        reinit!(fvu, coords, facetid)
